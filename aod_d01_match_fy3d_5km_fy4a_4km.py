...
def match_fy3d_5km_fy4a_4km(aod_fy3d_5km_file, geo_fy3d_5km_file, aod_fy4a_4km_file, aod_fy3d_5km_fy4a_4km_file,
                            longitude_range=None, latitude_range=None):
    print('<<<<< FY3D  : {}'.format(aod_fy3d_5km_file))
    print('<<<<< FY4A : {}'.format(aod_fy4a_4km_file))
    # 获取数据1
    try:
        fy3d_5km = AodFy3d5km(aod_fy3d_5km_file, geo_file=geo_fy3d_5km_file)
    ...
    if index_dist.sum() > 0:
        ...
        pd.DataFrame(result).to_csv(aod_fy3d_5km_fy4a_4km_file, index=False)
        print('>>>>> {}'.format(aod_fy3d_5km_fy4a_4km_file))
        return result
    else:
        print('匹配的数据量 < 0')
        return
        ...
        if fy3d_date_str in fy4a_file_dict:
            ...
            for fy3d_file in fy3d_files:
                for fy4a_file in fy4a_files:
                    ...
                    datetime_fy4a = datetime.strptime(ymdhms_fy4a, '%Y%m%d%H%M%S')
...
def t():
    aod_fy3d_5km_file = r"C:\F\shanghai\AOD\2FY3D_MERSI_L2_AOD-ORBT-1000M\FY3D_MERSI_ORBT_L2_AOD_MLT_NUL_20190102_0500_1000M_MS.HDF"
    geo_fy3d_5km_file = r"C:\F\shanghai\AOD\2FY3D-MERSI-L1-GEO1K\FY3D_MERSI_GBAL_L1_20190102_0500_GEO1K_MS.HDF"
    ...
    match_fy3d_5km_fy4a_4km(aod_fy3d_5km_file, geo_fy3d_5km_file, aod_modis_10km_file, aod_fy3d_5km_modis_10km_file,
                            longitude_range=longitude_range, latitude_range=latitude_range)
